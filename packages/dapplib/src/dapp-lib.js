...
module.exports = class DappLib {
	...
	// basicBeastCreateSet
	// calls transactions/basicbeast/create_set.cdc
	//
	// signer/proposer/authorizer: data.signer
	//
	static async basicBeastCreateSet(data) {
		let config = DappLib.getConfig();
		let result = await Blockchain.post(
			{
				config: config,
	...
	static async characterxCreateCharacter(data) {
		let result = await Blockchain.post(
			...
			'characterx_create_character',
			{
				...
				createdFrom_1: {
					value: parseInt(data.createdFrom_1),
					type: t.UInt64,
				},
				createdFrom_2: {
					value: parseInt(data.createdFrom_2),
					type: t.UInt64,
				},
				sex: { value: data.sex, type: t.String },
				race: { value: data.race, type: t.String },
				rarity: { value: data.rarity, type: t.String },
				lineage: DappLib.formatFlowDictionary(data.lineage, {
					key: t.String,
					value: t.Bool,
				}),
				bloodline: DappLib.formatFlowDictionary(data.bloodline, {
					key: t.String,
					value: t.Bool,
				}),
				element: DappLib.formatFlowDictionary(data.element, {
					key: t.String,
					value: t.Bool,
				}),
				traits: DappLib.formatFlowDictionary(data.traits, {
					key: t.String,
					value: t.String,
				}),
	...
	static async characterxFulfillPack(data) {
		...
		let result = await Blockchain.post(
			...
			{
				recipientAddr: { value: data.recipientAddr, type: t.Address },
				characterIDs: DappLib.formatFlowArray(
					data.characterIDs,
					t.UInt64
				),
			}
